...
export function CardLivro({ livro, onEdit, onDelete }: BookCardProps) {
  const imagemCapa = livro?.imagem_capa;
    ...
    <div className="group relative flex flex-col bg-white rounded-2xl shadow-sm border border-gray-100 hover:shadow-xl transition-all duration-300 overflow-hidden transform hover:-translate-y-1">
      ...
      <div className="flex-1 p-5 flex flex-col">
        ...
        <p
          ...
          title={livro.autores[0]?.id.toString()}
        >
          Autor: {livro.autores[0]?.nome || "Autor Desconhecido"}
          <span className="pl-3">Id: {livro.autores[0]?.id}</span>
        </p>
        <p className="text-sm text-gray-500 font-medium">
          Ano Publicação: {livro.ano_publicacao || "???"}
        </p>
